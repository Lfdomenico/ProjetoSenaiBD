...
public class GerSalario implements IntSalario {
		...
		try (Connection conn = ConexaoBD.getConexao()) {
			...
			if (rs.next()) {
				...
				try {
					...
					funcionario = new Funcionario(nome, cpf, setor, null);
	...
	public void consultarBeneficios(ArrayList<Funcionario> funcionarios) {
		...
		try (Connection conn = ConexaoBD.getConexao()) {
			String sql = "SELECT f.id, f.Nome, f.CPF, f.Salario AS SalarioBase, " + "s.nome AS SetorNome "
			...
			if (rs.next()) {
				...
				try {
					...
					funcionarioParaBeneficios = new Funcionario(nome, cpf, setor, null);
		...
		try (Connection conn = ConexaoBD.getConexao()) {
			...
			if (rs.next()) {
				...
				try {
					...
					funcionarioParaDemonstrativo = new Funcionario(nome, cpf, setor, null);
